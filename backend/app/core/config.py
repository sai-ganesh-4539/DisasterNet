...
def get_risk_threshold(hazard_type: Optional[str] = None, region: Optional[str] = None) -> int:
    """Get risk threshold for a specific hazard type or default"""
    if region:
        regional_specific = yaml_config.get('spatial.risk_thresholds.regional_specific', {})
...
def get_priority_weights() -> dict:
    ...
    return yaml_config.get('priority_scoring.weights', {
        'hazard_intensity': 0.35,
        'population_vulnerability': 0.30,
        'access_limitations': 0.20,
        'disaster_history': 0.15
