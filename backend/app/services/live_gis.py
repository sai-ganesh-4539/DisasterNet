...
def clip_bbox_to_india(south: float, west: float, north: float, east: float) -> Tuple[float, float, float, float]:
    s, w, n, e = INDIA_BBOX
...
def _grid_points(south: float, west: float, north: float, east: float, nx: int, ny: int) -> List[Tuple[float, float]]:
...
def fetch_meteo_batch(points: List[Tuple[float, float]]) -> List[Dict[str, Any]]:
    ...
    resp = requests.get(OPEN_METEO, params=params, timeout=25, headers={"User-Agent": USER_AGENT})
    ...
    for i, block in enumerate(blocks):
        lat, lon = points[i] if i < len(points) else (block.get("latitude"), block.get("longitude"))
        ...
        out.append(
            {
                ...
                "precipitation_current_mm": float(current.get("precipitation") or current.get("rain") or 0.0),
...
def fetch_river_discharge(points: List[Tuple[float, float]]) -> Dict[Tuple[float, float], float]:
...
def _slope_from_neighbors(cells: List[Dict[str, Any]], idx: int, nx: int, ny: int) -> float:
    ...
    rises = []
    for nb in neighbors:
        dist_m = _haversine_km(center["latitude"], center["longitude"], nb["latitude"], nb["longitude"]) * 1000.0
        if dist_m < 1:
            continue
...
def classify_hazard(elev: float, slope: float, precip_24: float, lon: float, discharge: float) -> str:
...
def score_grid_cells(cells: List[Dict[str, Any]], nx: int, ny: int, discharge_map: Dict[Tuple[float, float], float]) -> List[Dict[str, Any]]:
    ...
    for idx, cell in enumerate(cells):
        ...
        key = min(discharge_map.keys(), key=lambda k: _haversine_km(cell["latitude"], cell["longitude"], k[0], k[1]), default=None) if discharge_map else None
        discharge = discharge_map.get(key, 0.0) if key else 0.0
        hazard_type = classify_hazard(
            cell["elevation"], slope, cell["precipitation_24h_mm"], cell["longitude"], discharge
        )
        lithology = "SHALE" if slope > 20 else ("BASALT" if cell["elevation"] < 80 else "GRANITE")
        features = {
        ...
        pred["risk_level"] = (
            "CRITICAL" if risk >= 85 else "HIGH" if risk >= 70 else "MEDIUM" if risk >= 50 else "LOW"
        )
        scored.append(
            {
                **cell,
...
def build_red_zones(scored: List[Dict[str, Any]], dlat: float, dlon: float) -> List[Dict[str, Any]]:
    ...
    for cid, members in clusters.items():
        ...
        try:
            ...
            hull = MultiPoint([(m["longitude"], m["latitude"]) for m in members]).convex_hull
            if hull.geom_type == "Point":
                wkt = _cell_polygon(lat, lon, dlat, dlon)
            else:
                wkt = hull.buffer(max(dlat, dlon) * 0.45).wkt
        except Exception:
            wkt = _cell_polygon(lat, lon, dlat * len(members) ** 0.5, dlon * len(members) ** 0.5)
        ...
        zones.append(
            {
                ...
                "area_sq_km": round(max(dlat, 0.05) * 111 * max(dlon, 0.05) * 111 * len(members), 2),
            }
        )
    return zones
...
def fetch_habitations(bboxes: List[Tuple[float, float, float, float]]) -> List[Dict[str, Any]]:
    elements = []
    ...
    for el in elements:
        ...
        if population is None:
            population = {"city": 80000, "town": 18000, "village": 2200, "hamlet": 450}.get(place, 1500)
        state = tags.get("is_in:state") or tags.get("addr:state") or ""
        district = tags.get("addr:district") or tags.get("is_in:district") or tags.get("name") or ""
        places.append(
            {
                "osm_id": el.get("id"),
                "village_name": name,
...
def fetch_candidate_sites(bboxes: List[Tuple[float, float, float, float]]) -> List[Dict[str, Any]]:
    elements = []
    for south, west, north, east in bboxes:
        q = f"""
        [out:json][timeout:25];
    ...
    for el in elements:
        ...
        name = tags.get("name") or tags.get("name:en") or f"{amenity.title()} {el.get('id')}"
        ...
        sites.append(
            {
                ...
                "district_name": tags.get("addr:district") or tags.get("addr:city") or "",
...
def rank_habitations(places: List[Dict[str, Any]], zones: List[Dict[str, Any]], meteo_index: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for p in places:
        nearest_m = min(
            meteo_index,
            key=lambda c: _haversine_km(p["latitude"], p["longitude"], c["latitude"], c["longitude"]),
        ) if meteo_index else {}
        ...
        features = {
            ...
            "road_connectivity": "PAVED" if p.get("place") in ("city", "town") else "UNPAVED",
            ...
            "current_red_zone_id": zones[0]["zone_id"] if prox < 1.5 and zones else None,
            "hazard_history": [],
            "disaster_frequency_score": min(80.0, float(nearest_m.get("precipitation_72h_mm") or 0)),
        }
...
def evaluate_sites(sites: List[Dict[str, Any]], zones: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for s in sites:
        ...
        water_days = 14.0 if s.get("water_supply") or s["shelter_type"] == "HOSPITAL" else 8.0
        ...
        safe = suit["safety_evaluation"].get("safety_check_passed", True) and prox >= 1.0
        evaluated.append(
            {
                ...
                "medical_kits_available": 40 if s["medical_facility_depth"] == "FULL" else 12,
                ...
                "has_medical_facility": s["medical_facility_depth"] in ("FULL", "BASIC"),
                "has_kitchen": s["shelter_type"] in ("SCHOOL", "COMMUNITY_CENTER", "HOSPITAL"),
                "has_toilets": True,
    ...
    evaluated.sort(key=lambda r: (0 if r["is_in_safe_zone"] else 1, -r["available_capacity"]))
    return evaluated


def _focus_bboxes(zones: List[Dict[str, Any]], fallback: Tuple[float, float, float, float], limit: int = 3) -> List[Tuple[float, float, float, float]]:
    ...
    for z in sorted(zones, key=lambda x: x["risk_score"], reverse=True)[:limit]:
        pad = 0.28
        boxes.append(
            clip_bbox_to_india(z["latitude"] - pad, z["longitude"] - pad, z["latitude"] + pad, z["longitude"] + pad)
        )
    if not boxes:
        mid_lat = (south + north) / 2
        mid_lon = (west + east) / 2
        boxes.append(clip_bbox_to_india(mid_lat - 0.4, mid_lon - 0.4, mid_lat + 0.4, mid_lon + 0.4))
    return boxes


def inspect_point(lat: float, lon: float) -> Dict[str, Any]:
    """Score an arbitrary Indian coordinate from live meteorology + nearby OSM sites."""
    pad = 0.14
    south, west, north, east = clip_bbox_to_india(lat - pad, lon - pad, lat + pad, lon + pad)
    nx = ny = 3
    ...
    cell = min(scored, key=lambda c: _haversine_km(lat, lon, c["latitude"], c["longitude"]))
    ...
    dist = None
    if sites:
        nearest = min(sites, key=lambda s: _haversine_km(lat, lon, s["latitude"], s["longitude"]))
    ...
    return {
        ...
        "urgency": "IMMEDIATE" if cell["risk_score"] >= 70 else ("SHORT_TERM" if cell["is_red_zone"] else "MEDIUM_TERM"),
        "elevation": cell["elevation"],
...
def build_snapshot(
    ...
    cache_key = hashlib.md5(f"{south:.3f}:{west:.3f}:{north:.3f}:{east:.3f}:{nx}:{ny}".encode()).hexdigest()
    ...
    snapshot = {
        ...
        "summary": {
            ...
            "short_term_count": len([h for h in habitations if h["priority_category"] == "SHORT_TERM"]),
            "medium_term_count": len([h for h in habitations if h["priority_category"] == "MEDIUM_TERM"]),
            "shelter_count": len(shelters),
            "safe_shelter_count": len([s for s in shelters if s["is_in_safe_zone"]]),
            "available_beds": sum(s["available_capacity"] for s in shelters if s["is_in_safe_zone"]),
